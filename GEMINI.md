...
- **`hwp_com`** — OLE client library, depends on `hwp_core`
- **`hwp_addon`** — add-in framework, `HwpUserAction` trait, `#[repr(C)]` vtable structs
- **`hello_hwp_com`** — example binary using OLE client approach
...
Official SDK docs are in `hwp_sdk_doc/` (2025.04 edition):
