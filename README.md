...
SDK 설명에 더해 알아둘 IME, Undo, 팝업, C++ ABI 등의 경험과 구현상 가정은 [HWP 실전 노트](hwp_실전_노트.md)에 정리했습니다.
...
- **next-word 모드**: 커서가 단어 사이 공백에 있을 때 문서에서 직전 단어 다음에 등장하는 단어를 제안
- **매 호출 전체 스캔**: 액션마다 문서 전체에서 단어 목록을 새로 구성하고, 중복을 제거한 모든 후보를 문서 등장 순서대로 팝업에 전달
- **후보 선택**: 팝업에서 `↑` / `↓` / `Ctrl+/`로 순환, `Enter`로 확정, `Esc`로 취소
- **후보 현황**: 팝업 하단에 현재 선택 번호와 전체 후보 수를 표시하고, 화면 밖에 후보가 더 있으면 해당 방향의 `↑` / `↓` 표시
...
## 상표권 및 권리 고지 (Trademark & Rights Notice)

본 소프트웨어는 주식회사 한글과컴퓨터의 한글 문서 파일(.hwp) 공개 문서를 참고하여 개발되었습니다. (This software was developed by referring to the official HWP file format documentation provided by Hancom Inc.)
